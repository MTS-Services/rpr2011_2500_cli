    ...
    <PortalShell>
      ...
      <div className="max-w-2xl space-y-4">
        ...
        <div className="bg-white/90 rounded-xl border border-slate-100 overflow-hidden">
          ...
          {/* Admin-only notice */}
          <div className="mx-6 my-4 px-4 py-3 rounded-lg bg-amber-50 border border-amber-200 flex items-start gap-2.5">
            <Lock size={15} className="text-amber-600 shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-semibold text-amber-800">Edit Only by Admin</p>
          ...
          <div className="border-t border-slate-100">
            <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-slate-100">
              <div className="px-6 py-4">
                <p className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-3">Email</p>
                <p className="text-sm text-slate-700">{user?.email}</p>
              </div>
              <div className="px-6 py-4">
                <p className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-3">Address</p>
                <p className="text-sm font-semibold text-slate-700">{user?.address?.split(",")[0]}</p>
                <p className="text-sm text-slate-500">{user?.address?.split(",").slice(1).join(",").trim()}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 border-t border-slate-100">
              ...
              <div className="px-6 py-4">
                <p className="text-sm font-bold text-slate-700">{user?.ppsNumber}</p>
        ...
        <div className="bg-amber-50 rounded-xl border border-amber-200 overflow-hidden">
          ...
          <div className="px-5 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            <a
              href="/contact"
              className="shrink-0 inline-flex items-center gap-1.5 px-5 py-2.5 text-sm font-semibold text-white bg-teal-700 hover:bg-teal-800 rounded-lg transition"
            >
