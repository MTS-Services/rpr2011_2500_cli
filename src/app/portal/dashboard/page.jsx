  ...
  return (
    <PortalShell>
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold text-slate-800">Welcome Back, Joe</h1>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        {kpis.map(({ label, value, Icon, color }) => (
          <div key={label} className={`bg-white rounded-2xl border p-5 flex flex-col gap-3 shadow-sm ${color.split(" ")[2]}`}>
        ...
        <div className="lg:col-span-2 space-y-5">
            ...
            <div className="divide-y divide-slate-100">
              {alerts.map((a, i) => (
                <div key={i} className="flex items-center justify-between px-6 py-4">
                  ...
                  <span className={`text-sm font-semibold px-3 py-1.5 rounded-full whitespace-nowrap ml-4 ${a.badgeColor}`}>
                    {a.badge}
                  </span>
                </div>
              ))}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            ...
            <div className="px-6 py-4 border-t border-slate-100 text-center">
              <a href="/portal/properties" className="text-base text-teal-600 hover:text-teal-700 font-semibold flex items-center justify-center gap-1.5">
            ...
            {activity.map((a, i) => (
              <div key={i} className="flex gap-3.5 px-5 py-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-teal-600 font-medium mt-0.5">{a.property}</p>
                  <p className="text-sm text-slate-400 mt-0.5">{a.time}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="px-5 py-4 border-t border-slate-100 text-center">
            <a href="/portal/properties" className="text-base text-teal-600 hover:text-teal-700 font-semibold">
              View All Properties →
            </a>
          </div>
        </div>
