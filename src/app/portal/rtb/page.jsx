...
function LandlordRTBInner() {
  ...
  const handleUpdateRTB = async (e) => {
    ...
    try {
      ...
      await Swal.fire({ icon: "success", title: "Updated!", text: "RTB registration details updated successfully.", timer: 2000, showConfirmButton: false });
    } catch (err) {
      console.error("Error updating RTB:", err);
      await Swal.fire({ icon: "error", title: "Error", text: err.message || "Failed to update RTB registration." });
    } finally {
    ...
    <PortalShell>
      <div className="max-w-[1600px] mx-auto space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 tracking-tight">RTB Registration</h1>
          <p className="text-sm sm:text-base text-slate-500">Manage RTB registrations for your properties.</p>
        </div>
...
export default function LandlordRTBPage() {
  return (
